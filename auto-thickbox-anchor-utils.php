 ...
 *
 * @package Auto Thickbox Anchor Utils
 **/

class auto_thickbox_anchor_utils {

	private $auto_thickbox = null;
	/**
     * constructor
     */
    public function __construct( auto_thickbox $auto_thickbox, $inc_text_widgets = true ) {
...
//		$anchor = apply_filters( 'ob_filter_anchor', $anchor );
		$anchor = $this->auto_thickbox->filter( $anchor );
...
//		$anchor = apply_filters( 'filter_anchor', $anchor );
	...
	function escape_callback($match) {
		...
		$tag_id = "----escape_auto_thickbox_anchor_utils:" . md5($match[0]) . "----";
